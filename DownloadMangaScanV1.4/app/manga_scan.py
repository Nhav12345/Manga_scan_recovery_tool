...
path='C:\ScansManga'
try:
    os.mkdir(f"{path}\\verify")
    os.rmdir(f"{path}\\verify")
...
def get_name_fillable(_manga_name_):
    ...
    if len(manga_name)==0:
        ...
        cprint(f'Cette version du logiciel ({version}) ne peut gérer des noms-composés de plus de 3 mots.', 'red')
        return None
...
def download1(_manga, _chapter):
    ...
    for i in range(num_of_pages-1):
        n+=1
        sys.stdout.write('\r'+'['+'='*n+' '*(num_of_pages-(n+1))+'] '+f'{int(((n+1)/num_of_pages)*100)}%')
    ...
    if (os.listdir(f"C:\ScansManga\{_manga}\chapitre{_chapter}"))==[]:
        ...
        """if dictionnary.get(_manga, False)==False:
            dictionnary[_manga]=[]
            dictionnary[_manga].replace(int(_chapter))
        else:
            dictionnary[_manga].replace(int(_chapter))
        print(dictionnary)
        with open('annuaire.json', 'w+') as file:
...
while True:
    ...
    if manga=='':
        ...
        if "reader" in manga.lower():
            os.system('.\ms_reader.pyw')
        else:
            cprint("Veuillez respecter la syntaxe de la commande.", 'red')
    elif "get_lib" in manga.lower():
        if "-b" in manga.lower():
            for e in os.listdir(path):
                cprint(f"{e} :", 'green')
                for i in os.listdir(f"{path}\{e}"):
                    cprint(f"\t{i}", "blue")
        else:
            for e in os.listdir(path):
                cprint(f"{e} :", 'green')
    ...
    elif "delete" in manga.lower():
        if '-r' in manga.lower():
            dir_value=remove_space_from_list(re.split('delete|-r| ', manga.lower()))
            if len(dir_value)>0:
                rname=get_name_fillable(dir_value)
                try:
                    os.system(f'rmdir {path}\{rname} /s /q')
                except Exception as e:
                    print(e)
                    cprint(f"{rname}: ", end='')
                    cprint("Uninstallation Failed.", 'yellow')
                    pass
    else:
        if not get_user_connection_state(support, "cette fonctionnalité"):
            pass
