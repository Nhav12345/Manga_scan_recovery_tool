...
listofcommandmessage="help: cette commande affiche le message de base du logiciel\nis_available --[manga name][chapters]: détermine si un ou plusieurs chapitre(s) est disponible au telechargement\nstt: renvoie l'etat du support de téléchargement des scans\nget_version: renvoie la version de l'outil\n[manga name]: en entrant le nom d'un manga, vous pourrez télécharger un chapitre au choix"
...
while True:
    ...
    if manga=='':
        ...
        if not get_user_connection_state(support, "cette fonctionnalité"):
            ...
            print(manga)
            chapter=input("which chapter(s) do you want to download:")
            if "-" in chapter:
